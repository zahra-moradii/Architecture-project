...
public class ControlUnit extends Wrapper {
    ...
    public void initialize() {
        ...
        Link[] opCode = new Link[7];
        for(int index = 0; index <= 6; index++)
            opCode[index] = getInput(index);

        //************************************** R-types **************************************
        NAnd6 nAnd0 = new NAnd6("CUC_NAnd", "7X7",
                opCode[0],
                opCode[1],
                opCode[2],
                opCode[3],
        ...
        //************************************** BNQ **************************************
        Not not6 = new Not("CUC_NOT6", opCode[3]);
        NAnd6 nAnd2 = new NAnd6("CUC_NAnd2", "6X6",
                opCode[0],
                opCode[1],
                opCode[2],
                not6.getOutput(0),
                opCode[4],
                opCode[5]);
        CUControllersSet.Branch = nAnd2.getOutput(0);
        CUControllersSet.ALUOp0 = nAnd2.getOutput(0);

        Or or2 = new Or("CUC_OR0", and60.getOutput(0), and61.getOutput(0));
        CUControllersSet.ALUSrc = or2.getOutput(0);

        Or or3 = new Or("CUC_OR1", and60.getOutput(0), nAnd0.getOutput(0));
        Not not10 = new Not("CUC_NOT10", or3.getOutput(0));
        CUControllersSet.RegWrite = not10.getOutput(0);
